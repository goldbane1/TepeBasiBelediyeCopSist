    ...
    <div className="min-h-screen bg-[#f7fbf8] app-grid">
      <div className="flex min-h-screen">
        ...
        >
          ...
          {/* Aktif Oturum & Çıkış Kartı (Belediye Logosunun Hemen Altında) */}
          <div className="mt-3.5 rounded-2xl border border-white/15 bg-white/[.08] p-2.5 backdrop-blur-sm shadow-inner">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2.5 min-w-0 flex-1">
                <Avatar className="h-8 w-8 shrink-0 border border-emerald-300/40 shadow-xs">
                  <AvatarFallback className="bg-emerald-800 text-[11px] font-bold text-emerald-50">
                    {user.name?.slice(0, 2).toUpperCase() || "TB"}
                  </AvatarFallback>
                </Avatar>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs font-bold text-white leading-snug">{user.name || "Kullanıcı"}</p>
                  <p className="truncate text-[10px] font-semibold text-emerald-300 capitalize leading-none">{role}</p>
                </div>
              </div>
              <button
                onClick={logout}
        ...
        <section className="flex items-center bg-white p-6 sm:p-8 md:p-10 text-slate-900">
          <div className="w-full">
            ...
            <p className="mt-4 text-center text-[11px] text-slate-400 font-medium">
              Hesap bilgileriniz yönetim tarafından tanımlanmaktadır.
            </p>
          </div>
        </section>
      </div>
